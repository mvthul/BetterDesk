...
- **rdgen Build History panel:** live progress monitoring in the Generator UI — shows GitHub workflow run name (e.g. "Custom Windows Client Generator #1"), job-level progress counter ("2/5 jobs completed"), currently-running job name, determinate progress bar, GitHub Logs button, Build Portal link, and per-platform download buttons on completion. Polls every 15 seconds for active builds and stops automatically when all builds settle.
- **rdgen Presets in sidebar:** saved rdgen configurations now appear in the left bundle list alongside Support Agent and RdClient bundles, with a purple "RustDesk Generator" badge. Clicking a preset switches to the rdgen form and applies the configuration.
...
### Fixed
- **rdgen CSRF on GitHub Actions callbacks:** `/cleanzip`, `/save_custom_client`, `/get_zip`, `/get_png`, and `/generate` are now exempt from CSRF validation. These endpoints are called server-to-server by GitHub Actions (no browser session); the build UUID serves as the access token.
- **rdgen github_run_id not stored:** after `workflow_dispatch` (which returns 204 with no body), the service now asynchronously polls the GitHub Runs API for up to 60 seconds to match the new run by timestamp and stores its `github_run_id`, `log_url`, and real status (`queued`/`in_progress`) in the database.
- **rdgen initial status:** changed from the verbose string `"Starting generator...please wait"` to the clean value `"queued"`, consistent with GitHub Actions terminology and the terminal-status detection logic.
